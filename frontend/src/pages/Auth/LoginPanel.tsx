...
const LoginPanel: React.FC<{ onCancel?: () => void }> = ({ onCancel }) => {
  const { login } = useAuth();
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // success - UI will update via AuthContext
    } catch (err: any) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <button type="submit" disabled={loading} className="flex-1 bg-[#222831] text-white py-3 rounded-full font-black">{loading ? 'Signing in...' : 'Sign in'}</button>

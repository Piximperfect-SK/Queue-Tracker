...
const LogMonitorPage: React.FC = () => {
  ...
  return (
    <div className="h-full bg-white text-slate-950 font-mono rounded-4xl border border-white/40 flex flex-col overflow-hidden select-none shadow-xl">
      {/* Top Status Bar */}
      <div className="bg-[#222831] text-white px-6 py-3 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-4">
          <Terminal size={18} className="animate-pulse text-[#393E46]" />
          ...
          logs.map((log, i) => (
            <div key={i} className="flex gap-3 px-1 py-0.5 group">
              <span className="text-slate-400 shrink-0 select-none">[{log.timestamp}]</span>
                <span className="text-[#393E46] shrink-0 font-black">[{log.user}]</span>
              ...
              <span className={`break-all font-bold ${
                ...
                {i === logs.length - 1 && (
                <span className="w-1.5 h-4 bg-[#222831] animate-pulse inline-block align-middle ml-1" />
              )}
            </div>
          ))
        )}
